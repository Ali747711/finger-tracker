...
**The OK sign is deliberately kept apart from a click.** Both close the same thumb–index ring, so the three other fingers are the only difference — and they must be **clearly** extended, reaching well past their knuckle. That margin matters: `fingers_up` calls a finger "up" whenever its tip is merely above the joint, which an ordinary relaxed pinch satisfies, so a looser test made every pinch read as an OK sign and stopped clicking entirely. Only the OK detector's own verdict suppresses the click, never a raw finger count. While an OK sign is held the pinch neither clicks nor drags the cursor, and it can't arm a swipe either (an open palm requires the index extended, which an OK sign curls into the ring).
