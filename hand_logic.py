...
def fingers_up(landmarks, handedness='Right'):
    ...
    where y grows downward (MediaPipe convention).
    """
    if len(landmarks) != 21:
        raise ValueError(f'expected 21 landmarks, got {len(landmarks)}')

    up = {}
    # Fingers (not thumb): tip above PIP joint means extended.
    for name in ('index', 'middle', 'ring', 'pinky'):
        tip = FINGER_TIPS[name]
        up[name] = landmarks[tip][1] < landmarks[tip - 2][1]

    # Thumb extends sideways; compare tip x to IP-joint x, flipped per hand.
...
def count_fingers(landmarks, handedness='Right'):
    return sum(fingers_up(landmarks, handedness).values())
